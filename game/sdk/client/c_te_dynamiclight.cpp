...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEDynamicLight, DT_TEDynamicLight, CTEDynamicLight)
	...
	RecvPropInt( RECVINFO(m_iLightStyle)),
END_RECV_TABLE()
...
void TE_DynamicLight( IRecipientFilter& filter, float delay,
	const Vector* org, int r, int g, int b, int exponent, float radius, float time, float decay, int nLightIndex, int lightstyle = 0 )
{
#ifdef GE_DLL
	...
	if ( ToolsEnabled() && clienttools->IsInRecordingMode() )
	{
		...
		msg->SetInt( "lightstyle", lightstyle );
...
void C_TEDynamicLight::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	TE_DynamicLight( filter, 0.0f, &m_vecOrigin, r, g, b, exponent, m_fRadius, m_fTime, m_fDecay, LIGHT_INDEX_TE_DYNAMIC, m_iLightStyle );
}
...
void TE_DynamicLight( IRecipientFilter& filter, float delay, KeyValues *pKeyValues )
{
	...
	int lightstyle = pKeyValues->GetInt( "lightstyle", 0 );

	TE_DynamicLight( filter, 0.0f, &vecOrigin, c.r(), c.g(), c.b(), nExponent, 
		flRadius, flDuration, flDecay, nLightIndex, lightstyle );
}
