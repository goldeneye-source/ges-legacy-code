...
Color CBaseHudChat::GetTextColorForClient( TextColor colorNum, int clientIndex )
{
...
#ifdef GE_DLL
			if ( pClientScheme )
				c = pClientScheme->GetColor( "GEChatLocation", GetFgColor() );
			else
...
#ifdef GE_DLL
			if ( pClientScheme )
				c = pClientScheme->GetColor( "GEChatAchievement", GetFgColor() );
			else
				c = GetDefaultTextColor();
#endif
		}
	...
	default:
			...
			if ( pClientScheme )
			{
				c = pClientScheme->GetColor( "GEChatColor", GetDefaultTextColor() );
			}
			else
			{
				c = GetDefaultTextColor();
