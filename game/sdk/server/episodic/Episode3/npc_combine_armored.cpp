...
void CNPC_Combine_Armored::Spawn( void )
{
	...
	SetHealth( sk_combine_armored_health.GetInt() );
	SetMaxHealth( sk_combine_armored_health.GetInt() );
	SetKickDamage( sk_combine_armored_kick.GetInt() );
...
void CNPC_Combine_Armored::SpawnArmorPieces( void )
{
	...
	for ( size_t i = 0; i < ARRAYSIZE(ArmorPiecesPositions); i++ )
	{
