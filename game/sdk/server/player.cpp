...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( force == vec3_origin )
	{
		...
		if ( info.GetInflictor() && GetMoveType() == MOVETYPE_WALK && !attacker->IsSolidFlagSet(FSOLID_TRIGGER) )
		{
			...
			force = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage() );
		}
	}

	// Clamp force on alive players to 1000
	if ( m_iHealth > 0 && force.Length() > 1000.0f )
	{
...
// GE_DLL
// We disable this entire code block because GES implements a robust spawn system
#ifndef GE_DLL	
...
void CBasePlayer::Spawn( void )
{
	...
	{
		// GE_DLL: Add in no target support
