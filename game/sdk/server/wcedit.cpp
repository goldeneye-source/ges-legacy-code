...
void NWCEdit::CreateAINode( CBasePlayer *pPlayer )
{
	...
	else
	{
		...
		trace_t	tr;
		UTIL_TraceLine(startTrace,endTrace,MASK_NPCSOLID | CONTENTS_PLAYERCLIP, pPlayer, COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction != 1.0)
		{
	...
	if (bPositionValid)
	{
		...
		if (!g_pAINetworkManager->GetEditOps()->m_bAirEditMode)
		{
			UTIL_DropToFloor( testHull, MASK_NPCSOLID | CONTENTS_PLAYERCLIP );
		...
		UTIL_TraceHull( testHull->GetAbsOrigin(), vUpBit, NAI_Hull::Mins(hullType), 
			NAI_Hull::Maxs(hullType), MASK_NPCSOLID | CONTENTS_PLAYERCLIP, testHull, COLLISION_GROUP_NONE, &tr );
