...
//#if defined(GAME_DLL) && defined(GE_DLL)
	// register ourselves
...
#else
	C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();
	if ( pVictim->IsPlayer() && (!pLocalPlayer->InSameTeam( pVictim ) || pVictim->GetTeamNumber() == TEAM_UNASSIGNED) && pVictim != pAttacker )
		bVictimIsPlayerEnemy = true;
...
#ifdef _DEBUG
CON_COMMAND_F( achievement_reset_all, "Clears all achievements", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_reset, "<internal name> Clears specified achievement", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_status, "Shows status of all achievement", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_unlock, "<internal name> Unlocks achievement", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_unlock_all, "Unlocks all achievements", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_evaluate, "<internal name> Causes failable achievement to be evaluated", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_test_friend_count, "Counts the # of teammates on local player's friends list", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_test_clan_count, "Determines if specified # of teammates belong to same clan w/local player", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_mark_dirty, "Mark achievement data as dirty", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
...
CON_COMMAND_F( achievement_setprogress, "Set the progress of a named achievement", 0 )
{
	CAchievementMgr *pAchievementMgr = dynamic_cast<CAchievementMgr *>( engine->GetAchievementMgr() );
	if ( !pAchievementMgr )
		return;
