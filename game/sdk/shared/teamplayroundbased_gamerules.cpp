...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING( void )
{
	...
	if ( State_Get() == GR_STATE_RND_RUNNING && bAllowStalemateAtTimelimit && mp_timelimit.GetInt() > 0 && IsInPreMatch() == false )
	{
		if ( GetTimeLeft() <= 0.5 )
		{
			...
			{
				int iTeamScore = GetGlobalTeam(i)->GetMatchScore();
...
void CTeamplayRoundBasedRules::SetWinningTeam( int team, int iWinReason, bool bForceMapReset /* = true */, bool bSwitchTeams /* = false*/, bool bDontAddScore /* = false*/ )
{
	...
	{
		GetGlobalTeam( team )->AddMatchScore( TEAMPLAY_ROUND_WIN_SCORE );
...
bool CTeamplayRoundBasedRules::GetInfo_Custom(int valueType, pluginvariant &outValue, pluginvariant options)
{
	switch(valueType)
	{
	case TPRBGAMEINFO_GAMESTATE:
		{
			int mask = 0;
			//Check certain ones in order.
			if ( IsWaitingForTeams() )
				mask |= TPRB_STATE_WAITINGREADYSTART;

			if ( IsInWaitingForPlayers() )
				mask |= TPRB_STATE_WAITING;

			if ( InSetup() )
				mask |= TPRB_STATE_SETUP;

			if ( InRoundRestart() )
				mask |= TPRB_STATE_ROUNDRESET;

			if ( RoundHasBeenWon() )
				mask |= TPRB_STATE_ROUNDWON;

			if ( InOvertime() )
				mask |= TPRB_STATE_OVERTIME;

			if ( InStalemate() )
				mask |= TPRB_STATE_STALEMATE;

			if (State_Get() == GR_STATE_RND_RUNNING)
				mask |= TPRB_STATE_ACTIVE;

			outValue.SetInt( mask );
			return true;
		}
	}

	return BaseClass::GetInfo_Custom( valueType, outValue, options );
}
#endif //GAME_DLL - IGameInfo
