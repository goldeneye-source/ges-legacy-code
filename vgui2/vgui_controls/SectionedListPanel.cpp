...
void SectionedListPanel::PerformLayout()
{
	...
	if (m_iContentHeight > ctall && m_bVerticalScrollbarEnabled)
	{
		...
		m_pScrollBar->SetPos(cwide - 8 - 5, 0); //m_pScrollBar->GetWide()
		m_pScrollBar->SetSize(8, ctall - 2);
		m_pScrollBar->SetBgColor( Color(155,155,155,255) );
		m_pScrollBar->SetFgColor( Color(0,0,0,255) );
...
#endif
		m_pScrollBar->InvalidateLayout(true, false);
	...
	for (int sectionIndex = 0; sectionIndex < m_Sections.Size(); sectionIndex++)
	{
...
#ifdef GE_DLL
		// Used to determine the positioning of the next section
		sectionTall = 0;
#endif
		iStart = -1;
		iEnd = -1;
...
#ifdef GE_DLL
		// draw the header
		section.m_pHeader->SetBounds(x, headerY, wide, tall);
		...
		else
		{
			...
			for (int i = iStart; i <= iEnd; i++)
			{
				...
				// If this next item will make our section bigger then our Desired Height hide the rest of the items
				if ( section.m_iDesiredHeight > 0 && (sectionTall + m_iLineSpacing) > section.m_iDesiredHeight )
				{
					// Add this to our total height so that we will show a scrollbar if needed
					contentTall += m_iLineSpacing;
				...
				if ( y < (headerY - scheme()->GetProportionalScaledValue(2)) )
					item->SetVisible( false );
				else
					item->SetVisible( true );

				// If we have a section font, apply it to every item in that section
				if ( section.m_hFont != INVALID_FONT )
				{
					item->SetFont( section.m_hFont );
					item->ClearImages();
				...
				y += m_iLineSpacing;
				sectionTall += m_iLineSpacing;
			}
	#endif
		}

#ifdef GE_DLL
		// If we are not exactly at our desired height, make it so!
		if ( section.m_iDesiredHeight > 0 && sectionTall < section.m_iDesiredHeight )
			y += section.m_iDesiredHeight - sectionTall;

		// calculate total height
		contentTall += sectionTall;
#endif

		// Add a little gap between sections
		y += scheme()->GetProportionalScaledValue(5);
		headerY = y;
	}
...
#ifdef GE_DLL
	m_Sections[index].m_iDesiredHeight = 0;
...
#ifdef GE_DLL
void SectionedListPanel::SetHeaderDividerColor( int sectionID, Color color )
{
...
void SectionedListPanel::SetSectionHeight( int sectionID, int height )
{
	if ( !m_Sections.IsValidIndex(sectionID) && height >= 0 )
		return;

	m_Sections[sectionID].m_iDesiredHeight = scheme()->GetProportionalScaledValueEx(GetScheme(), height);
}
#endif
